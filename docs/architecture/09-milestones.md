...
| **M2 — Accounts and reliable ingestion** | Credential vault and 2FA; account setup and queued connection tests; choose IMAP adapter; tables for accounts, content, blobs, locations, checkpoints, failures, audit and thread assignment; bounded background sync | Synthetic server integration proves I1–I8, recovery and ownership; coverage/error status visible; credentials encrypted and redacted; no remote writes | Message organization UI, read/star write-back, external search, sending |
...
Security follows the feature that creates the risk: encrypted credentials and 2FA before real
accounts in M2; sanitization/SSRF defenses before any mail HTML/images in M3. M7 verifies and
