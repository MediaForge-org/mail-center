"""Remote-image network smoke using controlled upstream and real Laravel session check against the guarded test server on 8072.
...
with tempfile.TemporaryDirectory(prefix='mailcenter-session-browser-') as profile:
    ...
    try:
        ...
        with connect(tab['webSocketDebuggerUrl'], proxy=None, open_timeout=30) as ws:
            ...
            def source_requests():
                raw = subprocess.check_output(['docker','exec','mailcenter-remote-browser-test','sh','-c','cat /tmp/mailcenter-remote-requests.jsonl 2>/dev/null || true']).decode()
                return [json.loads(line) for line in raw.splitlines() if line]
            def click(text):
                js("[...document.querySelectorAll('.remote-image-controls button')].find(b=>b.textContent.trim().includes(" + json.dumps(text) + ")).click()")
            assert source_requests() == [], source_requests()
            assert not any('/api/image-proxy?' in u for u in browser_requests)
            assert js("[...document.querySelector('.reader-html').contentDocument.images].some(i=>i.src.includes('/inline/')&&i.naturalWidth===2)")
            click('Load images')
            wait_for("[...document.querySelector('.reader-html').contentDocument.images].some(i=>i.src.includes('/api/image-proxy?')&&i.naturalWidth===2)")
            assert len(source_requests()) == 1, source_requests()
            assert any('/api/image-proxy?' in u for u in browser_requests)
            assert all(not ('attacker.invalid' in u or 'fixture.test' in u) for u in browser_requests), browser_requests
            grant_url = js("document.querySelector('.reader-html').getAttribute('src')")
            # Switch through the SPA; one-time permission must not follow or survive reopening.
            js("[...document.querySelectorAll('.message-row')].find(b=>!b.classList.contains('message-selected')).click()")
            wait_for(f"document.querySelector('.reader-html')?.getAttribute('src') === '/api/messages/{other}/render'")
            wait_for("[...document.querySelector('.reader-html').contentDocument.images].some(i=>i.src.includes('/inline/')&&i.naturalWidth===2)")
            assert len(source_requests()) == 1
            click('Always load')
            wait_for("[...document.querySelector('.reader-html').contentDocument.images].some(i=>i.src.includes('/api/image-proxy?')&&i.naturalWidth===2)")
            assert len(source_requests()) == 2
            cdp('Page.reload')
            wait_for("document.querySelector('.reader-html')?.getAttribute('src')?.includes('images=allowed')")
            wait_for("[...document.querySelector('.reader-html').contentDocument.images].some(i=>i.src.includes('/api/image-proxy?')&&i.naturalWidth===2)")
            assert len(source_requests()) == 3
            proxy = js("[...document.querySelector('.reader-html').contentDocument.images].find(i=>i.src.includes('/api/image-proxy?')).getAttribute('src')")
            click('Stop always')
            wait_for(f"document.querySelector('.reader-html')?.getAttribute('src') === '/api/messages/{other}/render'")
            assert js('fetch('+json.dumps(proxy)+').then(r=>r.status)') == 404
            assert len(source_requests()) == 3
            for req in source_requests():
                assert req == {'user_agent':'MailCenter-Image/1.0','cookie':None,'authorization':None,'referer':None,'forwarded':None}, req
            assert all(not ('attacker.invalid' in u or 'fixture.test' in u) for u in browser_requests)
            assert logout() in [200,204]
            assert js('fetch('+json.dumps(proxy)+').then(r=>r.status)') == 401
            print('PASS: default zero upstream requests; Load once 1 server-side request; message navigation blocks; allowlist and reload use proxy; revocation/logout deny; CID works; zero direct browser requests; no identity headers.')
            cdp('Browser.close')
